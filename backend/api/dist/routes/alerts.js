...
// Validation schemas
const createAlertPolicySchema = zod_1.z.object({
    name: zod_1.z.string().min(1),
    description: zod_1.z.string().optional(),
    resourceId: zod_1.z.string().uuid(),
    metricName: zod_1.z.string(),
    condition: zod_1.z.enum(['gt', 'lt', 'eq', 'ne', 'gte', 'lte']),
    threshold: zod_1.z.number(),
    durationSeconds: zod_1.z.number().min(60).default(300),
    severity: zod_1.z.enum(['info', 'warning', 'critical']).default('warning'),
    channelIds: zod_1.z.array(zod_1.z.string().uuid()).optional(),
});
const createNotificationChannelSchema = zod_1.z.object({
    name: zod_1.z.string().min(1),
    type: zod_1.z.enum(['email', 'webhook', 'sms', 'slack']),
...
const alertRoutes = async (fastify) => {
    ...
    fastify.get('/policies', {
        ...
        const result = await (0, connection_1.query)(`SELECT ap.*, r.name as resource_name, r.type as resource_type,
       ...
       ORDER BY ap.created_at DESC`, [userId]);
        return reply.send({
    ...
    fastify.post('/policies', {
        ...
        try {
            ...
            const resourceCheck = await (0, connection_1.query)('SELECT id FROM ankercloud.resources WHERE id = $1 AND user_id = $2', [body.resourceId, userId]);
            if (resourceCheck.rows.length === 0) {
                return reply.code(404).send({ error: 'Resource not found' });
            }
            const policy = await (0, connection_1.transaction)(async (client) => {
                // Create policy
                const policyResult = await client.query(`INSERT INTO ankercloud.alert_policies
           ...
           RETURNING *`, [
                ...
                if (body.channelIds && body.channelIds.length > 0) {
                    for (const channelId of body.channelIds) {
                        await client.query(`INSERT INTO ankercloud.alert_policy_channels
               (alert_policy_id, channel_id)
               VALUES ($1, $2)`, [policy.id, channelId]);
                    }
                }
                return policy;
            });
            logger_1.logger.info(`Alert policy created: ${policy.id}`);
            ...
            logger_1.logger.error('Alert policy creation error:', error);
    ...
    fastify.delete('/policies/:id', {
        ...
        const result = await (0, connection_1.query)(`UPDATE ankercloud.alert_policies
       SET is_active = false
       WHERE id = $1 AND user_id = $2
       RETURNING id`, [policyId, userId]);
        if (result.rows.length === 0) {
            return reply.code(404).send({ error: 'Alert policy not found' });
        }
        logger_1.logger.info(`Alert policy deleted: ${policyId}`);
        return reply.send({
    ...
    fastify.get('/channels', {
        ...
        const userId = request.user.id;
        const result = await (0, connection_1.query)(`SELECT id, name, type, is_active, created_at
    ...
    fastify.post('/channels', {
        ...
        try {
            ...
            const result = await (0, connection_1.query)(`INSERT INTO ankercloud.notification_channels
         ...
         RETURNING id, name, type, is_active, created_at`, [
            ...
            logger_1.logger.info(`Notification channel created: ${channel.id}`);
            return reply.code(201).send({
                channel: {
                    id: channel.id,
                    name: channel.name,
            ...
            logger_1.logger.error('Channel creation error:', error);
            return reply.code(500).send({ error: 'Internal server error' });
        }
    });
    // Get incidents
    fastify.get('/incidents', {
        ...
        const userId = request.user.id;
        const { state, severity, resourceId } = request.query;
        let queryText = `
    ...
    fastify.post('/incidents/:id/acknowledge', {
        ...
        const incidentCheck = await (0, connection_1.query)(`SELECT i.id FROM ankercloud.incidents i
       JOIN ankercloud.alert_policies ap ON i.alert_policy_id = ap.id
       WHERE i.id = $1 AND ap.user_id = $2`, [incidentId, userId]);
        if (incidentCheck.rows.length === 0) {
            return reply.code(404).send({ error: 'Incident not found' });
        }
        await (0, connection_1.transaction)(async (client) => {
            // Update incident
            await client.query(`UPDATE ankercloud.incidents
         SET state = 'acknowledged',
             acknowledged_at = NOW(),
             acknowledged_by = $1,
             notes = COALESCE(notes || E'\\n' || $2, $2)
         WHERE id = $3`, [userId, notes, incidentId]);
            // Add to history
            await client.query(`INSERT INTO ankercloud.incident_history
        ...
        logger_1.logger.info(`Incident acknowledged: ${incidentId}`);
        return reply.send({
    ...
    fastify.post('/incidents/:id/resolve', {
       ...
       WHERE i.id = $1 AND ap.user_id = $2`, [incidentId, userId]);
        ...
        logger_1.logger.info(`Incident resolved: ${incidentId}`);
        return reply.send({
