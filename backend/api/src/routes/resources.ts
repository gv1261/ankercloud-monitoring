...
const createWebsiteSchema = z.object({
  ...
  checkIntervalSeconds: z.number().min(60).default(300),
...
const resourceRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    try {
      ...
      const resource = await transaction(async (client) => {
        ...
        const resourceResult = await client.query(
          `INSERT INTO ankercloud.resources
        ...
        await client.query(
          `INSERT INTO ankercloud.servers
           (resource_id, hostname, ip_address)
           VALUES ($1, $2, $3)`,
          [resource.id, body.hostname, body.ipAddress]
        );

        return resource;
      });

      logger.info(`Server resource created: ${resource.id}`);
  ...
  fastify.post('/websites', {
    ...
    try {
      ...
      const resource = await transaction(async (client) => {
        ...
        await client.query(
          `INSERT INTO ankercloud.websites
      ...
      logger.info(`Website resource created: ${resource.id}`);
      ...
      logger.error('Website creation error:', error);
      return reply.code(500).send({ error: 'Internal server error' });
    }
  });
  ...
  fastify.post('/networks', {
    ...
    try {
      ...
      const resource = await transaction(async (client) => {
        ...
        await client.query(
          `INSERT INTO ankercloud.network_checks
      ...
      logger.info(`Network resource created: ${resource.id}`);
      ...
      logger.error('Network creation error:', error);
      return reply.code(500).send({ error: 'Internal server error' });
    }
  });
  ...
  fastify.get('/:id/summary', {
    ...
    const result = await query(metricsQuery, [resourceId]);
